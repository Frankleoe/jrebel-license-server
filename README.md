...
### 方式一：Cloudflare（推荐，无需证书）

将域名解析到服务器后，在 Cloudflare 控制台开启 **Full SSL** 或 **Flexible SSL** 即可，无需手动配置证书。

```bash
# 克隆项目
git clone https://github.com/YOUR_USERNAME/jrebel-license-server.git
cd jrebel-license-server

# 修改 docker-compose.yml 中的 TOKEN（可选）
...
### 方式二：Let's Encrypt（自行管理证书）

```bash
# 生成 SSL 证书
certbot --nginx -d jrebel.afrank.cn
